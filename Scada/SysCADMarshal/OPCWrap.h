...
#ifndef WITHOPC
#pragma message("-------------------- WITHOPC NOT DEFINED--------------------")
#endif
#if WITHOPC
#include "opcda.h"
#include "opccomn.h"
...
#define TRACE_MSG(call, hr) \
  ...
  FormatMessage                                                    \
    (                                                              \
      ...
      FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetScode(hr), MAKELANGID   \
        (LANG_NEUTRAL, SUBLANG_NEUTRAL), msg, MAX_PATH, NULL       \
    );                                                             \
  /*TRACE(msg);*/   \
  /*TRACE(_T("\n"));*/   \
  /*LogError("",0,"hr=%lx %s", (hr), msg);*/   \
}
...
//*******************************************************************
typedef OInterfaceWrap<IOPCItemProperties, &IID_IOPCItemProperties>
  IOPCItemPropertiesWrap;

class OPCItemProperties :
  public IOPCItemPropertiesWrap
  {
public:
  OPCItemProperties(void) { }
  HRESULT QueryAvailableProperties
    ...
    { //ASSERT( IsOk() );
    HRESULT hr = m_pI->QueryAvailableProperties
      (
        szItemID,
        pdwCount,
        ppPropertyIDs,
  ...
  HRESULT GetItemProperties
    ...
    { //ASSERT( IsOk() );
    HRESULT hr = m_pI->GetItemProperties
      (
        szItemID,
        dwCount,
        pdwPropertyIDs,
...
//*******************************************************************
typedef OInterfaceWrap<IOPCServerPublicGroups, &IID_IOPCServerPublicGroups>
  IOPCServerPublicGroupsWrap;

class OPCServerPublicGroups :
  public IOPCServerPublicGroupsWrap
  {
public:
  OPCServerPublicGroups(void) { }

  // not yet implemented
  };

//*******************************************************************
typedef OInterfaceWrap
  <
    IOPCBrowseServerAddressSpace, &IID_IOPCBrowseServerAddressSpace
  > IOPCBrowseServerAddressSpaceWrap;

class OPCBrowseServerAddressSpace :
  public IOPCBrowseServerAddressSpaceWrap
  {
public:
  OPCBrowseServerAddressSpace(void) { }
  HRESULT QueryOrganization(OPCNAMESPACETYPE *pNameSpaceType)
    { //ASSERT( IsOk() );
    if(!IsOk()) return E_FAIL;

    HRESULT hr = m_pI->QueryOrganization(pNameSpaceType);
    if(FAILED(hr)) TRACE_MSG(_T("Browse::QueryOrganization"), hr);
    return hr;
    }
  ...
  HRESULT BrowseOPCItemIDs
    ...
    HRESULT hr = m_pI->BrowseOPCItemIDs
      (
        dwBrowseFilterType,
        szFilterCriteria,
        vtDataTypeFilter,
    ...
    if(FAILED(hr)) TRACE_MSG(_T("Browse::BrowseOPCItemIDs"), hr);
    return hr;
    }

  HRESULT GetItemID(LPWSTR szItemDataID, LPWSTR *szItemID)
    { //ASSERT( IsOk() );
    if(!IsOk()) return E_FAIL;

    HRESULT hr = m_pI->GetItemID(szItemDataID, szItemID);
    if(FAILED(hr)) TRACE_MSG(_T("Browse::GetItemID"), hr);
    return hr;
    }

  HRESULT BrowseAccessPaths(LPCWSTR szItemID, LPENUMSTRING *ppIEnumString)
    { //ASSERT( IsOk() );
    if(!IsOk()) return E_FAIL;

    HRESULT hr = m_pI->BrowseAccessPaths(szItemID, ppIEnumString);
    if(FAILED(hr)) TRACE_MSG(_T("Browse::BrowseAccessPaths"), hr);
    return hr;
    }
  };
...
//    Group Interfaces
//*******************************************************************
typedef OInterfaceWrap<IOPCGroupStateMgt, &IID_IOPCGroupStateMgt>
  IOPCGroupStateMgtWrap;

class OPCGroupStateMgt :
  public IOPCGroupStateMgtWrap
  {
public:
  OPCGroupStateMgt(void)  { }
  HRESULT GetState
          (
            DWORD     *pUpdateRate,
            BOOL      *pActive,
    ...
    if(FAILED(hr)) TRACE_MSG(_T("OPCGroupStateMgt::GetState"), hr);
    return hr;
    }
  ...
  HRESULT SetState
    ...
    if(FAILED(hr)) TRACE_MSG(_T("OPCGroupStateMgt::SetState"), hr);
    return hr;
    }
  ...
  HRESULT SetName(LPCWSTR szName)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("OPCGroupStateMgt::SetName"), hr);
    return hr;
    }

  HRESULT CloneGroup(LPCWSTR szName, REFIID riid, LPUNKNOWN *ppUnk)
    { //ASSERT( IsOk() );
    if(!IsOk()) return E_FAIL;

    HRESULT hr = m_pI->CloneGroup(szName, riid, ppUnk);
    if(FAILED(hr)) TRACE_MSG(_T("OPCGroupStateMgt::CloneGroup"), hr);
    return hr;
    }
  };

//*******************************************************************
typedef OInterfaceWrap<IOPCPublicGroupStateMgt, &IID_IOPCPublicGroupStateMgt>
  IOPCPublicGroupStateMgtWrap;

class OPCPublicGroupStateMgt :
  public IOPCPublicGroupStateMgtWrap
  {
public:
  OPCPublicGroupStateMgt(void)  { }
  HRESULT GetState(BOOL *pPublic)
    { //ASSERT( IsOk() );
    HRESULT hr = m_pI->GetState(pPublic);
    if(FAILED(hr)) TRACE_MSG(_T("IOPCPublicGroupStateMgt::GetState"), hr);
    return hr;
    }
  ...
  HRESULT MoveToPublic(void)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IOPCPublicGroupStateMgt::MoveToPublic"), hr);
    return hr;
    }
  };
...
public:
  ...
  HRESULT Read
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IOPCSyncIO::Read"), hr);
    return hr;
    }
  ...
  HRESULT Write
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IOPCSyncIO::Write"), hr);
    return hr;
    }
  };
...
public:
  ...
  HRESULT Read
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IOPCAsyncIO::Read"), hr);
    return hr;
    }
  ...
  HRESULT Write
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IOPCAsyncIO::Write"), hr);
    return hr;
    }

  HRESULT Refresh(DWORD dwConnection, OPCDATASOURCE dwSource, DWORD *pTransactionID)
    { //ASSERT( IsOk() );
    if(!IsOk()) return E_FAIL;

    HRESULT hr = m_pI->Refresh(dwConnection, dwSource, pTransactionID);
    if(FAILED(hr)) TRACE_MSG(_T("IOPCAsyncIO::Refresh"), hr);
    return hr;
    }
  ...
  HRESULT Cancel(DWORD dwTransactionID)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IOPCAsyncIO::Cancel"), hr);
    return hr;
    }
  };
...
public:
  ...
  HRESULT Read
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IOPCAsyncIO2::Read"), hr);
    return hr;
    }
  ...
  HRESULT Write
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IOPCAsyncIO2::Write"), hr);
    return hr;
    }

  HRESULT Refresh2(OPCDATASOURCE dwSource, DWORD pTransactionID, DWORD *pCancelID)
    { //ASSERT( IsOk() );
    if(!IsOk()) return E_FAIL;

    HRESULT hr = m_pI->Refresh2(dwSource, pTransactionID, pCancelID);
    if(FAILED(hr)) TRACE_MSG(_T("IOPCAsyncIO2::Refresh2"), hr);
    return hr;
    }
  ...
  HRESULT Cancel2(DWORD dwCancelID)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IOPCAsyncIO2::Cancel2"), hr);
    return hr;
    }
  ...
  HRESULT SetEnable(BOOL bEnable)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IOPCAsyncIO2::SetEnable"), hr);
    return hr;
    }
  ...
  HRESULT GetEnable(BOOL *pbEnable)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IOPCAsyncIO2::GetEnable"), hr);
    return hr;
    }
  };
...
public:
  ...
  HRESULT AddItems
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IOPCItemMgt::AddItems"), hr);
    return hr;
    }
  ...
  HRESULT ValidateItems
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IOPCItemMgt::ValidateItems"), hr);
    return hr;
    }

  HRESULT RemoveItems(DWORD dwNumItems, OPCHANDLE *phServer, HRESULT **ppErrors)
    { //ASSERT( IsOk() );
    if(!IsOk()) return E_FAIL;

    HRESULT hr = m_pI->RemoveItems(dwNumItems, phServer, ppErrors);
    if(FAILED(hr)) TRACE_MSG(_T("IOPCItemMgt::RemoveItems"), hr);
    return hr;
    }
  ...
  HRESULT SetActiveState
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IOPCItemMgt::SetActiveState"), hr);
    return hr;
    }
  ...
  HRESULT SetClientHandles
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IOPCItemMgt::SetClientHandles"), hr);
    return hr;
    }
  ...
  HRESULT SetDatatypes
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IOPCItemMgt::SetDatatypes"), hr);
    return hr;
    }
  ...
  HRESULT CreateEnumerator(REFIID riid, LPUNKNOWN *ppUnk)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IOPCItemMgt::CreateEnumerator"), hr);
    return hr;
    }
  };
...
public:
  ...
  HRESULT DAdvise
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IDataObject::DAdvise"), hr);
    return hr;
    }
  ...
  HRESULT DUnadvise(DWORD dwConnection)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IDataObject::DUnadvise"), hr);
    return hr;
    }
  ...
  HRESULT EnumDAdvise(IEnumSTATDATA **ppenumAdvise)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IDataObject::EnumDAdvise"), hr);
    return hr;
    }
  ...
  HRESULT GetData(FORMATETC *pformatetcIn, STGMEDIUM *pmedium)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IDataObject::GetData"), hr);
    return hr;
    }
  ...
  HRESULT QueryGetData(FORMATETC *pformatetc)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IDataObject::QueryGetData"), hr);
    return hr;
    }
  ...
  HRESULT GetDataHere(FORMATETC *pformatetc, STGMEDIUM *pmedium)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IDataObject::GetDataHere"), hr);
    return hr;
    }
  ...
  HRESULT GetCanonicalFormatEtc(FORMATETC *pformatectIn, FORMATETC *pformatetcOut)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IDataObject::GetCanonicalFormatEtc"), hr);
    return hr;
    }
  ...
  HRESULT SetData(FORMATETC *pformatetc, STGMEDIUM *pmedium, BOOL fRelease)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IDataObject::SetData"), hr);
    return hr;
    }
  ...
  HRESULT EnumFormatEtc(DWORD dwDirection, IEnumFORMATETC **ppenumFormatEtc)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IDataObject::EnumFormatEtc"), hr);
    return hr;
    }
  };

//*******************************************************************
typedef OInterfaceWrap<IEnumOPCItemAttributes, &IID_IEnumOPCItemAttributes>
  IEnumOPCItemAttributesWrap;

class EnumOPCItemAttributes :
  public IEnumOPCItemAttributesWrap
  {
public:
  EnumOPCItemAttributes(void) { }
  HRESULT Next(ULONG celt, OPCITEMATTRIBUTES **ppItemArray, ULONG *pceltFetched)
    { //ASSERT( IsOk() );
    if(!IsOk()) return E_FAIL;

    HRESULT hr = m_pI->Next(celt, ppItemArray, pceltFetched);
    if(FAILED(hr)) TRACE_MSG(_T("IEnumOPCItemAttributes::Next"), hr);
    return hr;
    }
  ...
  HRESULT Skip(ULONG celt)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IEnumOPCItemAttributes::Skip"), hr);
    return hr;
    }
  ...
  HRESULT Reset(void)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IEnumOPCItemAttributes::Reset"), hr);
    return hr;
    }
  ...
  HRESULT Clone(IEnumOPCItemAttributes **ppEnumItemAttributes)
    ...
    if(FAILED(hr)) TRACE_MSG(_T("IEnumOPCItemAttributes::Clone"), hr);
    return hr;
    }
  };
#endif
