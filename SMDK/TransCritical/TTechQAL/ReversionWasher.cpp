...
void ReversionWasher_UnitDef::GetOptions()
  {
  SetDefaultTag("RW", true);
  SetDrawing("WasherDecanter", ReversionWasherDraw);
  SetTreeDescription("Demo:Reversion Washer");
...
ReversionWasher::ReversionWasher(MUnitDefBase * pUnitDef, TaggedObject * pNd) : MBaseMethod(pUnitDef, pNd)
{
	//default values...
...
void ReversionWasher::BuildDataFields()
{
	...
	DD.Double("Mud.Level", "",    &dMudLevel, MF_PARAMETER|(bResTbyMud ? 0 : MF_NO_VIEW), MC_Frac);
...
void ReversionWasher::EvalProducts()
{
    ...
    QI1.AddF(SideStream, MP_All, 1.0);
...
ReversionStream::ReversionStream(MStream & ms_, MIBayer & mb_,  
								 double k0_, double dD_, bool bQALModel): MudStream(ms_), MudB(mb_),  dK0(k0_), dDelta(dD_)  
{
