...
static MInOutDefStruct s_IODefs[]=
{
	...
	{ "Feed1",          "Feed1",      0,   1,  5,    0,    1.0f,  MIO_In |MIO_Material },
	{ "Product1",       "Overflow",   1,   1,  1,    0,    1.0f,  MIO_Out|MIO_Material },
	{ "Product2",       "Underflow",  2,   1,  1,    0,    1.0f,  MIO_Out|MIO_Material },
	{ NULL },
};
...
void QALClassifier_UnitDef::GetOptions()
  {
  SetDefaultTag("CF", true);
  SetDrawing("SizeSeparation", QALClassifierDraw);
  SetTreeDescription("Demo:QAL Classifier");
...
void QALClassifier::EvalProducts() {
	...
	try
	{
		dTankArea = M_PI*dTankDiameter*dTankDiameter/4.0;
		FlwIOs.AddMixtureIn_Id(QI, 0);
		drhoSol = QI.Density(MP_Sol);
		dSolidsFlow = QI.MassFlow(MP_Sol);

		MStream & QO0 = FlwIOs[FlwIOs.First[1]].Stream;      // Product stream 1 - here the overflow
		MStream & QO1 = FlwIOs[FlwIOs.First[2]].Stream;      // Product stream 2 - the mud underflow


		// Set the outlet streams to the inlet streams (pass through) that if unexpected errors happen
		...
		if (IsNothing(QIB))
		{//Bayer properties interface not present
		...
		case 1:   // Specified underflow flow  - in this case use iteration
			...
			double r = dFuR/QI.VolumeFlow();   // Start by dividing the flows approximately
			QUnderflow.SetF(QI, MP_All, r);    // as required
...
double QALClassifier::EvalBalance(double x) {
	...
	for (int i=0; i<ic; i++) {
		...
		us += z*mp; // Total mass
		os += (1-z)*mp;
		//QOPSD.putMassPassing(i, (1-z)*mp);      // and divvy it up between the two streams.
...
double QALClassifier::EvalBalance(double x, double y) {   // General stream recombination
...
void QALClassifier::displayPSD(MStream &ms, int scrn)
{
	...
	for (int i=0; i<ic; i++) {
		switch (iPSD) {
			case 0:
				x = mpsd.getMassPassing(i);
				break;
			case 1: 
				x = mpsd.getFracPassing(i);
				break;
			case 2: 
				x = mpsd.getSizePassing(i);
				break;
		}
		dd[scrn][i]=x;
	}
