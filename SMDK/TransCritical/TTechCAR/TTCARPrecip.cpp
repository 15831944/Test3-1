...
void CPrecipitator_UnitDef::GetOptions()
{
  ...
  SetModelLicense(MLicense_HeatExchange|MLicense_Alumina | 0x00010000 );
};
...
void CPrecipitator::EvalPrecipRates(MStream & Prod, double T) 
{
  ...
  case GRM_White:  
    ...
    x[iALUMINA] = dGrowthRate*dSALin*dTankVol/1000.;
    x[iBOUND_SODA] = x[iBOUND_ORGANICS] = 0.0;
    break;
  case GRM_CAR_QAL: 
  ...
  case GRM_TTTest:
    dGrowthRate = dGrowthRateFactor*Sqr(dSSat);
    x[iALUMINA] =  dGrowthRate*dSALin*dTankVol;
    x[iBOUND_SODA] = x[iBOUND_ORGANICS]=0.0;
    break;
  }
