...
//---------------------------------------------------------------------------
DEFINE_TRANSFER_UNIT(CCARTubeDigester, "CARTubeDigester", DLL_GroupName)

void CCARTubeDigester_UnitDef::GetOptions()
  {
  SetDefaultTag("TR");
  SetDrawing("HeatExchange", Drw_CCARTubeDigester);
  SetTreeDescription("CAR:Tube Digester");
...
MToleranceBlock CCondensateFinder::s_Tol(TBF_Both, "CCARTubeDigester:CondensFinder", 0.0, 1.0e-12);
...
// actually fixed 
void CCARTubeDigester::DoLiveSteamHeater(MStream & ShellI, MStream & TubeI, MStream & ShellO, MStream & TubeO)
{
  TubeO = TubeI;
  ShellO = ShellI;
  ...
  if (TubeI.MassFlow()>0 && ShellI.MassFlow()>0 && ShellI.T > TubeI.T) {
    ...
    Cond = ShellI.MassFlow() - m_dQmVentRqd;
    ShellO.T = FTemp;
    double duty = ShellI.totHz()-ShellO.totHz();
...
void CCARTubeDigester::EvalProducts()
{
  try           
    ...
    case OM_LiveSteam:    DoLiveSteamHeater(ShellI, TubeI, ShellO, TubeO); break;
    }    
    m_dP = 0.0;
    if (m_lHxMode)
      flowRE(TubeI, TubeO);
    if (m_lOpMode)
      VentO.SetM(ShellI, MP_All, m_dQmVentRqd);
