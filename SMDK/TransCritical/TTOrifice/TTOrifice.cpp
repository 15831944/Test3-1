...
double CTTOrifice::massVelocity(MStream  ms, double pIn, double pOut) 
{
  ...
  double den = ms.Density()/dDensityCorrection;
  ...
  if (pIn < pSat) {
    ...
    double I1 = 2000.*(pIn-pOut)*(1./den1+4./den2+1./den3)/6.;
    return den3*sqrt(Sqr(dGIn/den) + I1);
  }
