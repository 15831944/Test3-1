...
//   Time-stamp: <2007-04-21 11:05:34 Rod Stephenson Transcritical Pty Ltd>
...
IMPLEMENT_SPECIEMODEL(AmiraBayer, "BayerSM", "*Bayer", "SMDK Generic Bayer Specie Model", DLL_GroupName)
...
double AmiraBayer::sm_dMinSolCp      = 1.046;
double AmiraBayer::sm_dBPE_Factor  = 1.;
double AmiraBayer::sm_dRho_Factor  = 1.;
double AmiraBayer::sm_dH2OTestFrac0  = 0.98;
double AmiraBayer::sm_dH2OTestFrac1  = 0.95;


double AmiraBayer::sm_dK1_BoundSodaCalc = 2.35 ;
double AmiraBayer::sm_dGrowth_TOC =	    0.00 ;
double AmiraBayer::sm_dGrowth_s =	    -1.00;
double AmiraBayer::sm_dGrowth_fc =	    -2.00;
double AmiraBayer::sm_dGrowth_eq =	    0    ;
double AmiraBayer::sm_dGrowth_n =	    1.70 ;
double AmiraBayer::sm_dGrowth_sol =	    1.00 ;
double AmiraBayer::sm_dGrowth_ssa =	    0.60 ;

byte AmiraBayer::sm_iASatMethod      = ASM_MAD99;
byte AmiraBayer::sm_iTSEMethod       = TSE_97;
...
AmiraBayer::AmiraBayer(TaggedObject *pNd) : LiqConcs25(this)
  {
  MInitialise(); // this must be called to initialise the SMDK library if neccessary
...
long AmiraBayer::DefinedPropertyInfo(long Index, MPropertyInfo & Info)
  ...
  switch(Inx)
    {
    case idASatMethod :
      Info.SetText("AmiraBayer Properties");
...
void AmiraBayer::LiquorComposition(bool DoIt) // Determine mass fractions in liquor from _liquid_ concentrations
  {
  ...
  const double density25 =CAmiraBayerConcs::LiquorDensEqn1(25.0, A, C, Ca , Cl, SO4, Ox, Org);
  ...
  if (tot > 1.0) 
    {
    Log.Message(MMsg_Warning, "AmiraBayer feed calculator: Bad liquor description - reduce concentrations!");
    M[Water]=1.0;
    M[CausticSoda]=0.0;
    M[Alumina]=0.0;
...
double AmiraBayer::DigAtoCEquil(double T_)
  {
...
double AmiraBayer::PFAtoCEquil(double T)
  {
  double Tc = K2C(T);
...
double AmiraBayer::PFSSN(double T) {
