...
#include "stdafx.h"
#define  __BULKSPLITTER_CPP
#include "bulksplitter.h"
...
static MInOutDefStruct s_IODefs[]=
  ...
  //  Desc;             Name;       Id; Rqd; Max; CnId, FracHgt;  Options;
    ...
    { "Product0",       "Prod0",  1,   1,   1,    0,    1.0f,  MIO_Out|MIO_Material },
    { "Product1",       "Prod1",  2,   1,   4,    0,    1.0f,  MIO_Out|MIO_Material },
    { NULL },
  };

static double Drw_BulkSplitter[] = { MDrw_Poly,  -12,6,  12,2,  12,-2, 0,-6, -12,2, -12,6,
                                        MDrw_Poly,  -12,4,  12,0,
                                        MDrw_Poly,  -12,2,  12,-2,
                                        MDrw_End };
...
DEFINE_TRANSFER_UNIT(CBulkSplitter, "BulkSplitter", DLL_GroupName)
void CBulkSplitter_UnitDef::GetOptions()
  {
  SetDefaultTag("EC");
  SetDrawing("BulkSplitter", Drw_BulkSplitter);
  SetTreeDescription("LimnDW:BulkSplitter");
...
CBulkSplitter::CBulkSplitter(MUnitDefBase * pUnitDef, TaggedObject * pNd) : MBaseMethod(pUnitDef, pNd)
  {
  gs_DWCfg.Initialise();
...
void CBulkSplitter::BuildDataFields()
  {
  m_DWParms.BuildDataFields(DD);
  }
bool CBulkSplitter::ExchangeDataFields()
  {
...
void CBulkSplitter::EvalProducts()
  {
  try
    {
    ...
    if (QI.IFExists<CLimnStream>())
      ...
      int Ret = _Model_DiamondWizard_BulkSplitter (gs_DWCfg.RowCount(),
					 		            			                      gs_DWCfg.ColCount(),
...
//====================================================================================
// Actions
bool CBulkSplitter::GetModelAction(CMdlActionArray & Acts)
  {
...
bool CBulkSplitter::SetModelAction(CMdlAction & Act)
  {
  switch (Act.iIndex)
    {
    case 0:
...
bool CBulkSplitter::GetModelGraphic(CMdlGraphicArray & Grfs)
  {
...
bool CBulkSplitter::OperateModelGraphic(CMdlGraphicWnd & Wnd, CMdlGraphic & Grf)
  {
