...
double  MSpecieDefn::Density(double T, double P, MVector * MVec)  { CDensityInfo C(SpModel::Fidelity(), SMO_DensNone, T, P, NULL, MVec?(MVec->SpMdl->SVData()):NULL); return m_pSp->DensityX(C) ? C.Density() : 0.0; };
double  MSpecieDefn::NDensity(MVector * MVec)                     { CDensityInfo C(SpModel::Fidelity(), SMO_DensNone, Norm_T, Norm_P, NULL, MVec?(MVec->SpMdl->SVData()):NULL); return m_pSp->DensityX(C) ? C.Density() : 0.0; };
double  MSpecieDefn::msVolume(double T, double P, MVector * MVec) { CDensityInfo C(SpModel::Fidelity(), SMO_DensNone, T, P, NULL, MVec?(MVec->SpMdl->SVData()):NULL); return m_pSp->DensityX(C) ? C.msVolume() : 0.0; };
double  MSpecieDefn::msNVolume(MVector * MVec)                    { CDensityInfo C(SpModel::Fidelity(), SMO_DensNone, Norm_T, Norm_P, NULL, MVec?(MVec->SpMdl->SVData()):NULL); return m_pSp->DensityX(C) ? C.msVolume() : 0.0; };
