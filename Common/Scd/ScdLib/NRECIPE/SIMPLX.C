...
//#include "nrecipe.h"
#define EPS 1.0e-6          
#define FREEALL { /*if (h) { fclose(h); h=NULL; }*/};
...
//      {
//      static int ii=0;
//      sprintf(dbgbuff, "d:\\temp\\xx\\a%07i.txt", ii++);
//      h=fopen(dbgbuff, "wt");
//      }
//    fprintf(h, "%s", dbgbuff);
//    }
...
void simplx(double **a, int m, int n, int m1, int m2, int m3, int *icase,
	int izrov[], int iposv[],	int *l1,	int *l2,	int *l3)
{
...
//	l3=ivector(1,m);
  ...
  //DumpA("start", dbgp, a, 1, m+2, 1, n+1, dmpfmt);
	...
	if (m2+m3) {
      ...
      //DumpA("simp1", dbgp, a, 1, m+2, 1, n+1, dmpfmt);
      ...
      //DumpA("simp2", dbgp, a, 1, m+2, 1, n+1, dmpfmt);
			if (ip == 0) {
				*icase = -1;
				FREEALL return;
			}
	one:	simp3(a,m+1,n,ip,kp);
      //DumpA("simp3", dbgp, a, 1, m+2, 1, n+1, dmpfmt);
