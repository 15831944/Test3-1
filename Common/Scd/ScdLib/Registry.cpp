...
void CRegistry::SetRegistryKey(HKEY hKey, LPCTSTR lpszRegistryKey, LPCTSTR lpszProfile)
  {
  ASSERT(m_pszRegistryKey == NULL);
  ASSERT(lpszRegistryKey != NULL);
  //ASSERT(m_pszAppName != NULL);
  //TODO: remove m_pszAppName 
  ...
  free((void*)m_pszProfileName);
  m_pszProfileName = _tcsdup(lpszProfile);
  AfxEnableMemoryTracking(bEnable);
  }


// returns key for m_hKey\"Software"\RegistryKey\ProfileName
...
HKEY CRegistry::GetAppRegistryKey()
  ...
  if (RegOpenKeyEx(m_hKey, _T("software"), 0, KEY_WRITE|KEY_READ,
...
// returns key for:
//      m_hKey\"Software"\RegistryKey\AppName\lpszSection
// creating it if it doesn't exist.
// responsibility of the caller to call RegCloseKey() on the returned HKEY
...
CString CRegistry::GetProfileString(LPCTSTR lpszSection, LPCTSTR lpszEntry,
  ...
  if (m_pszRegistryKey != NULL)
    ...
    DWORD dw = ::GetPrivateProfileString(lpszSection, lpszEntry,
      lpszDefault, szT, (sizeof(szT)/sizeof(szT[0])), m_pszProfileName);
    ASSERT(dw < 4095);
    return szT;
    }
  }
