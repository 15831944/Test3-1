...
flag SDBObject::DataXchg(DataChangeBlk & DCB)
  ...
  if (DCB.lHandle>=xidMolWt && DCB.lHandle<xidElemMolWt)
    ...
    switch (i*MaxSpecies+xidMolWt)
      ...
      case xidmlS:    DCB.D=dNAN;/*SDB[s].mlSf(FIDELITY(2), PARM(0), PARM(1), NULL, NULL)-
      ...
      case xidmsS:    DCB.D=dNAN;/*SDB[s].msSf(FIDELITY(2), PARM(0), PARM(1), NULL, NULL)-
      ...
      case xidRho:   
        if (!SDB[s].HasDensCalc())
          {
          CDensityInfo C(FIDELITY(2), SMOption_DensNone, PARM(0), PARM(1), NULL, NULL); 
  ...
  if (EI.PageNo==iPgElements)
    ...
    if (CurrentBlk(EI))
      ...
      switch (iId)
        ...
        case Id_Sf1:
          if (rSDBO.m_bShowMs)
            D=dNAN;/*SDB[iSp].msSf(rSDBO.m_bHiFidelity?1:0, rSDBO.m_dDisplayT, rSDBO.m_dDisplayP, NULL, NULL)-(SDB[iSp].m_FData[rSDBO.m_bHiFidelity?1:0].m_dCSf[0]/SDB[iSp].MoleWt());*/
          else
            D=dNAN;/*SDB[iSp].mlSf(rSDBO.m_bHiFidelity?1:0, rSDBO.m_dDisplayT, rSDBO.m_dDisplayP, NULL, NULL)-(SDB[iSp].m_FData[rSDBO.m_bHiFidelity?1:0].m_dCSf[0]);*/
          /*if (rSDBO.m_bShowMs)
        ...
        case Id_Rho1:
          if (SDB[iSp].HasDensCalc())
            ...
            {
            CDensityInfo C(rSDBO.m_bHiFidelity?1:0, SMOption_DensNone, rSDBO.m_dDisplayT, rSDBO.m_dDisplayP, NULL, NULL);
          ...
          if (1)
            ...
            if (SDB[iSp].HasDensCalc())
              ...
              if (rSDBO.m_iDisplaySolvent==iSolvent)
                ...
                const double AppDensity = SI.Evaluate(CDensityInfo(rSDBO.m_bHiFidelity?1:0, SMOption_DensNone, rSDBO.m_dDisplayT, rSDBO.m_dDisplayP, NULL, NULL), 1.0-rSDBO.m_dDisplayMF, rSDBO.m_dDisplayMF);
                RhoFmt.FormatFloat(RhoCnv.Human(AppDensity), T);
                Str += T;
#endif
