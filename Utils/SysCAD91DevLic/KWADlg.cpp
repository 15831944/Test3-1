...
BOOL CAboutDlg::OnInitDialog() 
  ...
  char Buff[128];
  sprintf(Buff, "SysCAD Developer Licensing Utility (Version %d.%d)", KWA_VERINFO_V0, KWA_VERINFO_V1);
  #ifdef _DEBUG
  strcat(Buff, " (Debug)");
  #endif
...
void CKwaDlg::UpdateInfo()
  ...
  if (Lic.Licensed())
    ...
    sprintf(Buff1, "Version %d.%d", KWA_VERINFO_V0, KWA_VERINFO_V1);
    #ifdef _DEBUG
    strcat(Buff1, " (Debug)");
    #endif
    if (Lic.DaysLeft()==CK_InfiniteDays)
      ...
      sprintf(Buff, "%s  License expires in %d days", Buff1, Lic.DaysLeft());
    SetDlgItemText(IDC_TXT_VERSION, Buff);
    if (Lic.SMDK_Units() && Lic.SMDK_Props())
      sprintf(Buff1, "Units and Properties");
    else if (Lic.SMDK_Units())
      sprintf(Buff1, "Units");
    else if (Lic.SMDK_Props())
      sprintf(Buff1, "Properties");
    else
      sprintf(Buff1, "Other");
    sprintf(Buff, "Developer License (SMDK %s) is OK", Buff1);
    ...
    {
    SetDlgItemText(IDC_TXT_LICENSE, "Developer License not found");
    SetDlgItemText(IDC_TXT_USERS, "Software is NOT licensed");//"Users:0");
    }
  }
