...
* THIRD PARTY WITHOUT THE EXPRESS WRITTEN CONSENT OF PURECOMPONENTS     *
*                                                                       *
...
namespace PureComponents.TreeView
{
	...
	public class TreeView : Control, ISupportInitialize
	{
		...
		/// <summary>
		/// Defiens whether teh focus rectangle is being shown
		/// </summary>
		protected bool m_bShowFocusRectangle = true;
        ...
        /// </summary>
        /// <param name="contextMenu"></param>
        private void ContextMenuExtensionTreeView(ContextMenuStrip contextMenu)
        {
            if (this.ContextMenuEditing == true)
            {
                if (IsInContextMenuStrip(contextMenu, m_oContextMenuString.AddNode) == false)
                    if (AllowAdding == true)
                        contextMenu.Items.Add(m_oContextMenuString.AddNode, null, new EventHandler(OnContextMenuEditingAddNode));
                if (IsInContextMenuStrip(contextMenu, m_oContextMenuString.Copy) == false)
                    if (AllowEditing == true)
                        contextMenu.Items.Add(m_oContextMenuString.Copy, null, new EventHandler(OnContextMenuEditingCopy));
                if (IsInContextMenuStrip(contextMenu, m_oContextMenuString.Paste) == false)
                    if (AllowEditing == true)
                        contextMenu.Items.Add(m_oContextMenuString.Paste, null, new EventHandler(OnContextMenuEditingPaste));
            }

            if (this.ContextMenuArranging == true)
            {
                if (IsInContextMenuStrip(contextMenu, m_oContextMenuString.Expand) == false)
                    contextMenu.Items.Add(m_oContextMenuString.Expand, null, new EventHandler(OnContextMenuArrangingExpand));
                if (IsInContextMenuStrip(contextMenu, m_oContextMenuString.Collapse) == false)
                    contextMenu.Items.Add(m_oContextMenuString.Collapse, null, new EventHandler(OnContextMenuArrangingCollapse));            
            }

            if (this.ContextMenuXmlOperations == true)
            {
                if (IsInContextMenuStrip(contextMenu, m_oContextMenuString.SaveXml) == false)
                    contextMenu.Items.Add(m_oContextMenuString.SaveXml, null, new EventHandler(OnContextMenuXmlOperationsSave));
                if (IsInContextMenuStrip(contextMenu, m_oContextMenuString.LoadXml) == false)
                    contextMenu.Items.Add(m_oContextMenuString.LoadXml, null, new EventHandler(OnContextMenuXmlOperationsLoad));
            }
        }
	    ...
	    /// <param name="s"></param>
	    /// <returns></returns>
	    private bool IsInContextMenuStrip(ContextMenuStrip menu, string s)
	    {
		...
		internal void InvokePaintNodeCustomFlag(Graphics graphics, Node node, int alpha, Rectangle rect)
		{
			OnPaintNodeCustomFlag(graphics, node, alpha, rect);
		}
		...
		/// <param name="alpha"></param>
		/// <param name="rect"></param>
		protected virtual void OnPaintNodeCustomFlag(Graphics graphics, Node node, int alpha, Rectangle rect)
		{
			if (PaintNodeCustomFlag != null) PaintNodeCustomFlag(graphics, node, alpha, rect);
		}
		
		/// <summary>
		/// 
		/// </summary>
		public delegate void PaintNodeCustomFlagEventHandler(Graphics graphics, Node node, int alpha, Rectangle rect);
		
		/// <summary>
		/// 
		/// </summary>
		public event PaintNodeCustomFlagEventHandler PaintNodeCustomFlag;
...
//				}	
		...
		protected override void OnPaintBackground(PaintEventArgs pevent)
		{
			...
			if (this.Style.BackColor != Color.Transparent)
			{
				...
				//
				// now paint the flags area if needed
				//
				if (this.Flags == true)
				{
					//
					//
					//
					if (this.Style.FlagsBackColor != Color.Transparent)
					{
						AreaPainter.PaintRectangle(
							pevent.Graphics, 1, 1, 15, Height - 2, this.Style.FlagsFillStyle,
							this.Style.FlagsBackColor, this.Style.FlagsFadeColor, BorderStyle.None, Color.White);
					}
				}
			}
		}
...
//				MessageBox.Show(e.ToString());
		...
		protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
		{
			...
			if (keyData == Keys.Left)
			{
				...
				if (oSelNode.IsExpanded == false || oSelNode.Nodes.Count == 0 || oSelNode.CanCollapse == false)
					return true;
		...
		protected override void OnDoubleClick(EventArgs e)
		{
			...
			foreach (Rectangle r in m_mapRectToSubItem.Keys)
			{
				...
				{
					if (oNode.IsExpanded == true && (oNode.Nodes.Count > 0 || oNode.Panel != null) && oNode.CanCollapse == true)
						oNode.Collapse();
...
//				return;
		...
		/// <summary>
		/// Gets or sets whether the control should show focus rectangle
		/// </summary>
		[Category("Behavior")]
		[Description("Gets or sets whether the control should show focus rectangle.")]
		[DefaultValue(true)]
		public bool FocusRectangle
		{
			get
			{
				return m_bShowFocusRectangle;
			}
			
			set
			{
				m_bShowFocusRectangle = value;
