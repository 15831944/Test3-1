...
{ 
...
#ifdef __ACAD11__
    INT *templat = dx0_entity_template ( DX_ENTITY_ID(entity), 
    ...
    if ( DX_FILE_BLOCKS_CLOSED(file) && 
        ...
        RETURN ( FALSE ) ;
    if ( templat == NULL || entity == NULL ) 
        RETURN ( FALSE ) ;
    if ( DXF_ENTITY_IS_INSERT(entity) && 
        !dxf_check_block ( file, DXF_INSERT_BLOCK_GET(entity) ) )
        RETURN ( FALSE ) ;
    ...
    if ( !dx0_put_object ( file, DX_ENTITY_ANY(entity), templat, 
        put_6, put_62 ) )
        RETURN ( FALSE ) ;
    DX_FILE_ENT_LAYER_FREE(file); //kga 23/9/97 : prevent memory leak
    DX_FILE_ENT_LAYER_SET(file, DXF_ENTITY_LAYER_GET(entity) ) ;
...
{ 
    ...
    if( cnt == 0 ){ // no vertices on the pline
        RETURN ( FALSE ) ;
    }
    if( cnt == 1 ){  // to stop a pline of 1 vertex gong into the file
        printf(" Polyline with less than two vert being put\n");
...
{ 
    ...
    DML_WALK_LIST ( blist, item ) {
        ...
        if ( ( strcmp ( DX_BLOCKPOS_NAME_GET(bpos), blockname ) == 0 ) )
            RETURN ( TRUE ) ;
    }
    RETURN ( FALSE ) ;
/*
...
REAL target_pt[3], dir_vec[3] ;
{
    ...
    entity = dxf_create_viewport ( file, ctr_pt, width, height, 
        id, status, target_pt, dir_vec ) ;
#else
    entity = dxf_create_viewport ( ctr_pt, width, height, 
        id, status, target_pt, dir_vec ) ;
#endif
