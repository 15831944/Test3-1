...
BOOLEAN *near_tan ;
{
    ...
    REAL rad1, rad2, q1, q2, u, v, u1[2], u2[2] ;
    INT x [2] ;
    ...
    if ( !C2_ARC_ZERO_RAD(arc1) && !C2_ARC_ZERO_RAD(arc2) && 
        ...
        rad1 = c2r_rad ( C2_ARC_SEG(arc1) ) ;
        rad2 = c2r_rad ( C2_ARC_SEG(arc2) ) ;
        C2V_SUB ( ctr2, ctr1, vec ) ;
        if ( C2V_IS_SMALL ( vec ) ) 
            RETURN ( IS_SMALL(rad1-rad2) ? -1 : 0 ) ;
        vec1[0] = vec[0] / rad1 ;
        vec1[1] = vec[1] / rad1 ;
        ...
        if ( near_tan != NULL && n == 2 ) {
            u = C2V_NORM ( vec ) ; 
            if ( IS_SMALL ( u - rad1 - rad2 ) || 
                 IS_SMALL ( u - rad1 + rad2 ) || 
                 IS_SMALL ( u + rad1 - rad2 ) ) {
                near_tan[0] = TRUE ;
                near_tan[1] = TRUE ;
            }
