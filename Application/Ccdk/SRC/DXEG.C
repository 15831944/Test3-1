    ...
    if ( code == DXF_BAD_CODE ) {
        ...
        if ( entity == NULL ) 
            ...
            if ( !dx0_pline_vtcs ( file, entity ) ) {
                file->file_status |= DXF_PLINENOVERTS;    /* added by mhm */
                dxf_free_entity ( entity ) ;            
                RETURN ( FALSE ) ;
            }
        }
            ...
            if ( DXF_TEXT_STYLE_GET(entity)   || strlen ( DXF_TEXT_STYLE_GET(entity) ) == 0 )
                 DXF_TEXT_STYLE_SET(entity, "STANDARD" ) ;
        }
        else if ( DXF_ENTITY_IS_ATTDEF(entity) ) {
            if ( DXF_ATTDEF_STYLE_GET(entity) || strlen ( DXF_ATTDEF_STYLE_GET(entity) ) == 0 )
                 DXF_ATTDEF_STYLE_SET(entity, "STANDARD" ) ;
        }
        else if ( DXF_ENTITY_IS_ATTRIB(entity) ) {
            if ( DXF_ATTRIB_STYLE_GET(entity) || strlen ( DXF_ATTRIB_STYLE_GET(entity) ) == 0 )
                DXF_ATTRIB_STYLE_SET(entity, "STANDARD" ) ;
        }
        RETURN ( TRUE ) ;
    }
    else
...
//    dx0_strcpy ( DX_FILTER_LAYER(filter), layer ) ;
      DX_FILTER_LAYER_SET(filter, layer ) ;
}
...
DXF_FILTER filter ;
{
    dx0_strfree(DX_FILTER_LAYER_PTR(filter));
