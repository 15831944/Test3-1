...
{ 
    ...
    if ( DX_FILE_BLOCKS_POS_DEF(file) ) 
        ...
        if ( !dx0_find_after ( file, 0, "SECTION", 2, "BLOCKS", NULL ) ) {
            dx0_rewind ( file ) ;
            if ( !dx0_find_after ( file, 2, "SECTION", 0, "BLOCKS", NULL ) ) {
                (void)dx0_setpos ( file, &pos0 ) ;
                RETURN ( FALSE ) ;
            }
        }
...
{ 
    ...
    DML_WALK_LIST ( DX_FILE_BLOCKPOS_LIST(file), item ) {
        ...
        if ( strcmpi ( DX_BLOCKPOS_NAME_GET((DXF_BLOCKPOS)(DML_RECORD(item))), 
            blockname ) == 0 )
    ...
    while ( TRUE ) {
        ...
        if ( strcmp ( string, "BLOCK" ) == 0 ) {
            ...
            if ( !dx0_find ( file, 2, NULL, NULL ) ||
                 !dx0_get_string ( file, string2 ) ||
                 !DX_BLOCKPOS_NAME_SET(blockpos,string2) ) {

// mhm                !dx0_get_string ( file, DX_BLOCKPOS_NAME_GET(blockpos) ) ) {
