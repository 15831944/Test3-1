...
#ifdef SPLINE
/*-------------------------------------------------------------------------*/
BBS_PRIVATE DXF_ENTITY dx3_spline_to_entity ( file, spline, parm0, parm1 ) 
/*-------------------------------------------------------------------------*/
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
/*!!!!!!!! (C) Copyright 1989, 1990, 1991 Building Block Software   !!!!!!!*/
/*!!!!!!!!                    All rights reserved                   !!!!!!!*/
...
PARM parm0, parm1 ;
{
    ...
    if ( ( parm0 != NULL && c3n_trimmed0 ( spline, parm0 ) ) ||
        ( parm1 != NULL && c3n_trimmed1 ( spline, parm1 ) ) ) {

        C3_NURB tspline = c3n_trim ( spline, parm0, parm1 ) ;
        pline = dx3_spline_to_entity ( file, tspline, NULL, NULL ) ;
    ...
    vertex = dx3_vertex_to_entity ( file, C3_NURB_CTPT(spline)[0], 0.0, 
    ...
    m = splinesegs * ( n - C3_NURB_D(spline) + 1 ) ;
    PARM_J(&parm) = 0 ;
    ...
    for ( i=0, PARM_T(&parm) = 0.0 ; i <= m ; i++, PARM_T(&parm) += h ) {
        ...
        vertex = dx3_vertex_to_entity ( file, pt, 0.0, DXF_VERTEX_SPL_FIT, 
    ...
    for ( i=1 ; i<n ; i++ ) {
        vertex = dx3_vertex_to_entity ( file, C3_NURB_CTPT(spline)[i], 0.0, 
            DXF_VERTEX_SPL_FR, planar, x, y, extrusion ) ;
        if ( vertex == NULL ) 
            RETURN ( NULL ) ;
        if ( dml_append_data ( DXF_PLINE_VLIST(pline), vertex ) == NULL )
...
/*-------------------------------------------------------------------------*/
BBS_PRIVATE DXF_ENTITY dx3_vertex_to_entity ( file, pt, bulge, flags, 
    planar, x, y, extrusion ) 
/*-------------------------------------------------------------------------*/
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
/*!!!!!!!! (C) Copyright 1989, 1990, 1991 Building Block Software   !!!!!!!*/
