...
STATIC REAL fillet_crtr __(( PT2 DUMMY0 , PT2 DUMMY1 [] )) ;
...
PARM parm1, parm2 ;
{
    ...
    for ( count = 0 ; count < C2CF_MAX_COUNT ; count++ ) {
        ...
        if ( IS_ZERO(u) ) {
            ...
            d1 = fillet_crtr ( c1, x1 ) + fillet_crtr ( c1, x2 ) ;
            d2 = fillet_crtr ( c2, x1 ) + fillet_crtr ( c2, x2 ) ;
            if ( d1 < d2 ) {
                C2V_COPY ( c1, ctr ) ;
            }
            else {
...
if ( DIS_LEVEL == -1 || DISPLAY <= DIS_LEVEL ) {
    point_color ( x1[0], .03, 11 ) ;
    point_color ( x2[0], .03, 12 ) ;
    point_color ( ctr, .04, 13 ) ;
    getch();
}
else
    DISPLAY-- ;
