...
REAL elevation, extrusion[3] ;
{
    ...
    DXF_DESC_LAYER_SET(descriptor, NULL ) ;
    DXF_DESC_HANDLE_SET(descriptor, NULL ) ;
    DXF_DESC_LTYPE_SET(descriptor, NULL ) ;
    ...
    DXF_DESC_LAYER_SET(descriptor, layer ) ;
    DXF_DESC_HANDLE_SET(descriptor, NULL ) ;
    DXF_DESC_LTYPE_SET(descriptor, ltype ) ;
...
{
    dx0_strfree(DXF_DESC_LAYER_PTR(descriptor));  
    dx0_strfree(DXF_DESC_HANDLE_PTR(descriptor));  
    dx0_strfree(DXF_DESC_LTYPE_PTR(descriptor));   
...
STRING text, style_name ;
{
    ...
    DXF_TEXT_VALUE_SET(entity, text );

    //dx0_strass ( &DXF_TEXT_VALUE(entity), text ) ; // mhmmem



    DXF_TEXT_STYLE_SET(entity, 
...
//    dx0_strcpy ( DXF_SHAPE_NAME(entity), name ) ;
    DXF_SHAPE_NAME_SET(entity, name ) ;
    RETURN ( entity ) ;
}
...
//    dx0_strcpy ( DXF_INSERT_BLOCK(entity), block ) ;
    DXF_INSERT_BLOCK_SET(entity, block ) ;
...
INT flags, len ;
{
    ...
    DXF_ATTDEF_VALUE_SET(entity, text ) ; // mhmmem
//    dx0_strass ( &DXF_ATTDEF_VALUE(entity), text ) ; // mhmmem
//    dx0_strcpy ( DXF_ATTDEF_PROMPT(entity), prompt ) ;
    DXF_ATTDEF_PROMPT_SET(entity, prompt ) ;
//    dx0_strcpy ( DXF_ATTDEF_TAG(entity), tag ) ;
    DXF_ATTDEF_TAG_SET(entity, tag ) ;


//    strcpy ( DXF_ATTDEF_STYLE(entity), 
//        style_name == NULL || style_name[0]=='\0' ? "STANDARD" : style_name ) ;

    DXF_ATTDEF_STYLE_SET(entity,style_name == NULL || style_name[0]=='\0' ? "STANDARD" : style_name ) ;
...
INT flags, len ;
{
    ...
    DXF_ATTRIB_VALUE_SET(entity, text ) ; // mhmmem

    DXF_ATTRIB_VALUE_SET(entity, text ) ; // mhmmem
//    dx0_strass ( &DXF_ATTRIB_VALUE(entity), text ) ; // mhmmem


//    dx0_strcpy ( DXF_ATTRIB_TAG(entity), tag ) ;
    DXF_ATTRIB_TAG_SET(entity, tag ) ;
//    strcpy ( DXF_ATTRIB_STYLE(entity), 
    DXF_ATTRIB_STYLE_SET(entity, 
        style_name == NULL || style_name[0]=='\0' ? "STANDARD" : style_name ) ;
...
//    dx0_strcpy ( DXF_DIM_NAME(entity), name ) ;
    DXF_DIM_NAME_SET(entity , name ) ;

    DXF_DIM_TEXT_SET(entity, text ) ; // mhmmem
//    dx0_strass ( &DXF_DIM_TEXT(entity), text ) ; // mhmmem
...
REAL target_pt[3], dir_vec[3] ;
{
    ...
    DXF_VIEWPORT_PAPER_SPACE_SET(entity, "ACAD" ) ;
    DXF_VIEWPORT_DATA_BEGIN_SET(entity, "MVIEW" ) ;
    DXF_VIEWPORT_WND_DATA_SET(entity, "{" ) ;

    DXF_VIEWPORT_VERSION(entity) = 16 ;
    DXF_VIEWPORT_FROZEN_BEGIN_SET(entity, "{" ) ;
    DXF_VIEWPORT_LAYER_LIST(entity) = NULL ;
    DXF_VIEWPORT_FROZEN_END_SET(entity, "}" ) ;
    DXF_VIEWPORT_DATA_END_SET(entity, "}" ) ;
    RETURN ( entity ) ;
}
...
INT flags, len ;
{
    ...
    DXF_ATTRIB_ROT_ANG(entity) = ang ;
    DXF_ATTRIB_X_SCALE(entity) = scale ;
    DXF_ATTRIB_VALUE_SET(entity, text ) ;
