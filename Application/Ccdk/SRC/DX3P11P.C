...
/*-------------------------------------------------------------------------*/
BBS_PRIVATE DXF_ENTITY dx3_pcurve_to_entity ( file, pcurve, t0, t1 ) 
/*-------------------------------------------------------------------------*/
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
/*!!!!!!!!  (C) Copyright 1989, 1990, 1991 Building Block Software  !!!!!!!*/
/*!!!!!!!!                    All rights reserved                   !!!!!!!*/
...
{ 
    ...
    if ( t0 > BBS_ZERO || t1 < (REAL) c3p_n(pcurve) - 1.0 - BBS_ZERO ) {
        ...
        pline = dx3_pcurve_to_entity ( file, tcurve, 
    ...
    if ( DXF_PLINE_VLIST(pline) == NULL )
        RETURN ( NULL ) ;
    dx3_pc_buffer_to_entity ( file, C3_PCURVE_BUFFER(pcurve), pline, planar, 
        x, y, extrusion ) ;
    C3_WALK_PCURVE ( pcurve, item ) 
        dx3_pc_buffer_to_entity ( file, DML_RECORD(item), pline, planar, 
            x, y, extrusion ) ;

    if ( !closed && c3p_ept1 ( pcurve, p ) ) {
        vertex = dx3_vertex_to_entity ( file, p, 0.0, 0, planar, 
            x, y, extrusion ) ;
        if ( vertex == NULL ) 
            RETURN ( NULL ) ;
