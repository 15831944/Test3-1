  ...
  public:
    ...
    COleInfoReportGrfItem(LPTSTR Drawing, LPTSTR ClassId, LPTSTR Tag) :  \
      ...
      if (stricmp(ClassId, "pipe-1")==0)
        m_iLnk=1;
      else if (stricmp(ClassId, "CtrlLink")==0)
        m_iLnk=2;
      else if (stricmp(ClassId, "E_Cable")==0)
        m_iLnk=3;
      else
        m_iLnk=0;
      };
...
static int SpGrfRptTest(void * p, void * q)
  ...
  Cmp=stricmp(r1->m_sDrawing, r2->m_sDrawing);
  if (Cmp==0)
    {
    if (r1->m_iLnk==0 && r2->m_iLnk==0)
      Cmp=stricmp(r1->m_sClassId, r2->m_sClassId);
    else
      Cmp=r1->m_iLnk-r2->m_iLnk;
    }
  if (Cmp==0)
    Cmp=stricmp(r1->m_sTag, r2->m_sTag);
  return Cmp<0;
  };
